...
export async function streamAgentTurn(args: {
  ...
  res: Response;
  userEmail: string;
  /** MAS endpoint name. For Genie-only demos (like Streamline), leave empty
   * and set genieSpaceId instead. See caredesk.ts AgentContext. */
  masEndpointName: string;
  /** Genie space id. For Genie-only demos, set this; leave masEndpointName empty.
   * See caredesk.ts AgentContext. */
  genieSpaceId: string;
  ...
  modelError: ModelErrorDetail | null;
}> {
  ...
  // Captured by the OpenAI fetch shim in caredesk.ts on any non-2xx
  ...
  const modelError: { current: ModelErrorDetail | null } = {
    current: null,
  };
  ...
  try {
    ...
    for await (const ev of stream) {
      ...
      // through harmlessly.
      if (typedEv.type === 'raw_model_stream_event') {
        const data = typedEv.data;
        if (!data) continue;
      ...
      if (typedEv.type === 'run_item_stream_event') {
        const item = typedEv.item;
        if (!item) continue;
        const raw = item.rawItem;
        if (!raw) continue;
        if (typedEv.name === 'tool_called' && raw.type === 'function_call') {
          thinking.push({
            kind: 'tool_call',
            callId: raw.callId ?? '',
            name: raw.name ?? '',
        ...
        } else if (typedEv.name === 'tool_output') {
          sawToolOutput = true;
          console.debug(
