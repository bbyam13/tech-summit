...
type ChatConfig = {
   ...
   * there into the AgentContext used by caredesk.ts. Leave empty for Genie-only demos. */
...
export async function handleChatStream(args: {
  ...
  let out: Awaited<ReturnType<typeof streamAgentTurn>>;
  try {
    out = await streamAgentTurn({
      ...
      // (caredesk.ts `setDefaultOpenAIClient`). `databricks-gpt-5-4` is the
