...
const tablesSchema = z.object({
  subscriberPosition: z.string().min(1),
  openAtrisk: z.string().min(1),
  offers: z.string().min(1),
  // Optional — set to the ML predictions table name if the demo has one,
  // empty string or omitted otherwise. db/sync.ts skips when falsy.
  retentionRecommendations: z.string().optional(),
});
...
await createApp({
  ...
  // Streamline uses Genie. If your demo uses MAS instead, swap genieSpaceId
  // here for masEndpointName and update caredesk.ts AgentContext + makeTools() accordingly.
  if (!appConfig.genieSpaceId && !appConfig.masEndpointName) {
    console.warn(
      '[boot] Both genieSpaceId and masEndpointName are empty — the agent won\'t have an ask_data tool. Set one in config/app.json.',
    );
  }
